...
func TestEmbedCarriesEveryMigration(t *testing.T) {
	...
	if len(entries) != 40 {
		t.Fatalf("expected 40 files (20 up + 20 down), got %d", len(entries))
	}
	for i := 1; i <= 20; i++ {
		found := 0
		for _, e := range entries {
			if len(e.Name()) >= 6 && e.Name()[:6] == fmt.Sprintf("%06d", i) {
				found++
			}
		}
		if found != 2 {
			t.Errorf("version %06d has %d files, want 2 (up+down)", i, found)
		}
	}
}
```
