...
def build_prompt(seed: dict, bank: dict[str, dict], retry_feedback: str | None = None) -> tuple[str, str]:
    ...
    system = (
        ...
        "5. `chunks`: split `en` into natural spoken chunks that reproduce `en` EXACTLY under "
        "this join rule: chunks join with a single space by default, EXCEPT a chunk that STARTS "
        "with one of , . ? ! attaches directly to the previous chunk with NO space (English never "
        "puts a space before those). So the sentence's trailing punctuation mark is its OWN final "
        "chunk, not glued onto the last word. Example: en \"Let me check your wristband.\" -> "
        "chunks [\"Let me check your wristband\", \".\"]. Another: en \"Wristband, right?\" -> "
        "chunks [\"Wristband\", \", right\", \"?\"]. Get this exactly right — it is mechanically "
        "checked (verify_lesson_content.py V5 / Go JoinChunks) and a mismatch fails the whole "
        "sentence.\n"
