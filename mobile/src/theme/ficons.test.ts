...
// CI must actually RUN the comparison, not skip it quietly.
//
// The skip exists so a checkout without the submodule still has a green suite. That is
// right, and it is also how the comparison spent eight commits never running anywhere
// but one laptop. So: wherever the submodule IS present, the four parity tests must
// have executed — and this asserts that from inside the same run, using the same
// condition they gate on.
test('the parity comparison is not silently skipped where it can run', () => {
  if (!REF_PRESENT) {
    // Absent is legitimate — but say which environment chose to skip, so a CI log that
    // stopped comparing does not read like a CI log that compared.
    console.warn('[ficons] parity comparison skipped: docs/dlc submodule absent');
    return;
  }
  // Present: the handoff parsed and carried a full set. If this passes, so did the
  // four itWithHandoff tests above — they read the same `handoff` object.
  expect(Object.keys(handoff.icons).length).toBe(88);
  expect(Object.keys(handoff.emoji).length).toBeGreaterThan(100);
});
