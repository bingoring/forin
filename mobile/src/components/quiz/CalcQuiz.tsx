    ...
    <QuizShell
      ...
      footer={
        checked && correct
          ...
          : checked
            ? <View style={{ flex: 1 }}><NbButton variant="paper" full onPress={() => { setChecked(false); setEntry(''); }}>{t('quiz.retry')}</NbButton></View>
            : (
              <>
                {!!c.secondCheck && <View style={{ flex: 1, justifyContent: 'center' }}><Text style={{ fontFamily: nbFonts.body, fontSize: 9, color: nb.soft }}>2nd check: <Text style={{ fontFamily: nbFonts.hand, color: C }}>{c.secondCheck}</Text></Text></View>}
                <View style={{ flex: 1 }}><NbButton variant="ink" full iconColor={nb.paper} disabled={!entry} onPress={() => setChecked(true)}>{rich ? t('quiz.doubleCheck') : t('quiz.submitCalc')}</NbButton></View>
              </>
            )
      }
    >
      ...
      {rich ? (
        ...
        <Shadowed offset={3}>
          <View style={{ backgroundColor: nb.paper, borderWidth: 1.5, borderColor: nb.paperEdge, padding: 12 }}>
            {(c.given ?? []).map((g, i) => (
              <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 3, borderBottomWidth: 1, borderBottomColor: 'rgba(62,54,43,.18)', borderStyle: 'dashed' }}>
                <Text style={{ fontFamily: nbFonts.body, fontSize: 12, color: C }}>{g.label}</Text>
...
function DosageBody({ c, entry, checked, correct }: { c: NonNullable<QuizDetail['content']>; entry: string; checked: boolean; correct: boolean }) {
  ...
  return (
    <>
      ...
      {!!o && (
        <Shadowed offset={3}>
          <View style={{ backgroundColor: nb.paper, borderWidth: 1.5, borderColor: nb.paperEdge, marginBottom: 10 }}>
            <View style={{ backgroundColor: 'rgba(249,227,123,.5)', borderBottomWidth: 2.5, borderBottomColor: C, paddingVertical: 4, paddingHorizontal: 8, flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={{ fontFamily: nbFonts.hand, fontSize: 13.5, color: C }}>ORDER · {o.id}</Text>
              <Text style={{ fontFamily: nbFonts.hand, fontSize: 12.2, color: C }}>{o.prescriber} · {o.time}</Text>
            </View>
            <View style={{ paddingVertical: 8, paddingHorizontal: 10 }}>
              {!!o.patient && <Text style={{ fontFamily: nbFonts.body, fontSize: 12, color: C }}><Text style={{ fontFamily: nbFonts.hand }}>Patient: </Text>{o.patient}</Text>}
              {!!o.drug && <Text style={{ fontFamily: nbFonts.body, fontSize: 12, color: C, marginTop: 3 }}><Text style={{ backgroundColor: 'rgba(249,227,123,.5)', fontFamily: nbFonts.hand }}> {o.drug} </Text></Text>}
            </View>
          </View>
        </Shadowed>
      )}
      ...
      <View style={{ flexDirection: 'row', gap: 10, alignItems: 'flex-start' }}>
        {!!v && (
          <Shadowed offset={3} style={{ width: 100 }}>
            <View style={{ backgroundColor: nb.paper, borderWidth: 1.5, borderColor: nb.paperEdge, padding: 6, paddingTop: 10 }}>
              <View style={{ position: 'absolute', top: -7, left: 4, backgroundColor: nb.paper, borderWidth: 1.3, borderColor: nb.ink, paddingHorizontal: 4 }}>
                <Text style={{ fontFamily: nbFonts.hand, fontSize: 10.8, color: C }}>ON HAND</Text>
              </View>
              <VialArt />
              <Text style={{ fontFamily: nbFonts.hand, fontSize: 13.5, color: C, textAlign: 'center', marginTop: 4 }}>{v.drug}</Text>
              <Text style={{ fontFamily: nbFonts.hand, fontSize: 16.2, color: '#DC2626', textAlign: 'center', marginTop: 2 }}>{v.concentration}</Text>
              {!!v.size && <Text style={{ fontFamily: nbFonts.body, fontSize: 9, color: nb.soft, textAlign: 'center', marginTop: 1 }}>{v.size}</Text>}
            </View>
          </Shadowed>
        )}
        ...
        <Shadowed offset={3} style={{ flex: 1 }}>
          <View style={{ backgroundColor: nb.paper, borderWidth: 1.5, borderColor: nb.paperEdge, paddingVertical: 8, paddingHorizontal: 10 }}>
            <QuizSection label="FORMULA" />
            {/* D/H × Q = ? */}
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              <Fraction top="D" bottom="H" />
              <Text style={{ fontFamily: nbFonts.hand, fontSize: 18.9, color: C }}>× Q =</Text>
              <View style={{ backgroundColor: entry ? 'rgba(168,217,151,.4)' : 'rgba(249,227,123,.5)' + '55', borderWidth: 1.4, borderColor: nb.ink, paddingVertical: 2, paddingHorizontal: 10, minWidth: 44, alignItems: 'center' }}>
                <Text style={{ fontFamily: nbFonts.hand, fontSize: 18.9, color: C }}>{entry || '?'}</Text>
              </View>
            </View>
            ...
            {checked && (
              <View style={{ marginTop: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, flexWrap: 'wrap' }}>
                ...
                <Text style={{ fontFamily: nbFonts.hand, fontSize: 17.6, color: C }}>× {c.perQty} =</Text>
                <View style={{ backgroundColor: correct ? 'rgba(249,227,123,.5)' : '#FFF0EC', borderWidth: 1.4, borderColor: nb.ink, paddingVertical: 3, paddingHorizontal: 8 }}>
                  <Text style={{ fontFamily: nbFonts.hand, fontSize: 18.9, color: C }}>{c.answer} {c.answerUnit}</Text>
                </View>
              </View>
            )}
            <View style={{ marginTop: 8, paddingTop: 8, borderTopWidth: 1.5, borderTopColor: 'rgba(62,54,43,.18)', borderStyle: 'dashed' }}>
              <Text style={{ fontFamily: nbFonts.body, fontSize: 9, color: nb.soft, lineHeight: 13 }}>
                <Text style={{ fontFamily: nbFonts.hand, color: C }}>D</Text> Desired · <Text style={{ fontFamily: nbFonts.hand, color: C }}>H</Text> On Hand · <Text style={{ fontFamily: nbFonts.hand, color: C }}>Q</Text> per Quantity
              </Text>
            </View>
          </View>
        </Shadowed>
        ...
        <Shadowed offset={3} style={{ marginTop: 10 }}>
          <View style={{ backgroundColor: nb.paper, borderWidth: 1.5, borderColor: nb.paperEdge, paddingVertical: 8, paddingHorizontal: 10 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <Text style={{ fontFamily: nbFonts.hand, fontSize: 13.5, color: C }}>주사기 눈금 ({syringeMax} mL)</Text>
              <View style={{ backgroundColor: 'rgba(168,217,151,.4)', borderWidth: 1.3, borderColor: nb.ink, paddingHorizontal: 5, paddingVertical: 1 }}>
                <Text style={{ fontFamily: nbFonts.hand, fontSize: 14.9, color: C }}>{c.answer} {c.answerUnit}</Text>
              </View>
            </View>
      ...
      <Shadowed offset={2} style={{ marginTop: 10 }}>
        <View style={{ backgroundColor: '#FEF3C7', borderWidth: 1.4, borderColor: nb.ink, paddingVertical: 7, paddingHorizontal: 9, flexDirection: 'row', gap: 6 }}>
          <View style={{ backgroundColor: '#F59E0B', borderWidth: 1.3, borderColor: nb.ink, paddingHorizontal: 5, alignSelf: 'flex-start' }}>
            <Text style={{ fontFamily: nbFonts.hand, fontSize: 12.2, color: nb.paper }}>5R</Text>
          </View>
          <Text style={{ flex: 1, fontFamily: nbFonts.body, fontSize: 10, color: C, lineHeight: 15 }}>
            <Text style={{ fontFamily: nbFonts.hand }}>5 Rights: </Text>Right Patient · Drug · Dose · Route · Time — 모두 확인했나요?
          </Text>
        </View>
      </Shadowed>
    </>
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
      <View style={{ alignItems: 'center' }}>
        <Text style={{ fontFamily: nbFonts.hand, fontSize: 17.6, color: col, paddingHorizontal: 3 }}>{top}</Text>
        <View style={{ height: 2, backgroundColor: C, alignSelf: 'stretch' }} />
        <Text style={{ fontFamily: nbFonts.hand, fontSize: 17.6, color: col, paddingHorizontal: 3 }}>{bottom}</Text>
      </View>
      {!!sub && <Text style={{ fontFamily: nbFonts.body, fontSize: 9, color: nb.soft }}>{sub}</Text>}
    </View>
  );
}
...
function SyringeScale({ fill, label }: { fill: number; label: string }) {
  return (
    <View style={{ height: 30, justifyContent: 'center' }}>
      ...
      <View style={{ position: 'absolute', left: 0, top: 12, width: 6, height: 6, backgroundColor: '#475569', borderWidth: 1.3, borderColor: nb.ink }} />
      <View style={{ position: 'absolute', right: 0, top: 14, width: 6, height: 2, backgroundColor: C }} />
      <View style={{ height: 16, backgroundColor: nb.paper, borderWidth: 1.4, borderColor: nb.ink, marginHorizontal: 6, position: 'relative' }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 6, marginTop: 2 }}>
        <Text style={{ fontFamily: nbFonts.hand, fontSize: 10.8, color: nb.soft }}>0</Text>
        <Text style={{ fontFamily: nbFonts.hand, fontSize: 10.8, color: C }}>{label}</Text>
        <Text style={{ fontFamily: nbFonts.hand, fontSize: 10.8, color: nb.soft }}>MAX</Text>
      </View>
    </View>
  );
}
